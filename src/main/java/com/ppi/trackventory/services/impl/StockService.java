...
public class StockService {
    ...
    public Optional<Stock> getStockById(Long storeId, String variationCode) {
        ...
        ProductVariation variation = productVariationService.getVariationByCode(variationCode).orElse(null);
        ...
        if (stock.getQuantity() <= 5) {
            try {
                emailService.sendHtmlEmail("fabian_bernal23222@elpoli.edu.co", "Prueba", "<!DOCTYPE html>\r\n"
                        ...
                        + "        <p>Hola,</p>\r\n"
                        + "        <p>Queremos informarte que el producto con referencia <strong>" + stock.getId().getVariationStk().getCode() + " (" + stock.getId().getVariationStk().getProductVrt().getReference() + ")</strong> en la tienda <strong>" + stock.getId().getStoreStk().getCode() + "</strong> tiene un nivel de stock bajo. Solo quedan <strong>" + stock.getQuantity() + " unidades</strong>.</p>\r\n"
                        ...
                        + "            <p>&copy; 2024 Trackventory. Todos los derechos reservados.</p>\r\n"
                        + "        </div>\r\n"
                        + "    </div>\r\n"
                        + "</body>\r\n"
                        + "</html>\r\n"
 ...
 // Obtener stocks por tienda
    public List<Stock> getStockByStoreId(Long storeId) {
        Store store = storeService.getStoreById(storeId).orElse(null);
        if (store != null) {
            return stockRepository.findByIdStoreStk(store);
        }
        return List.of();
    }
    ...
    public List<Stock> getStockByVariationCode(String variationCode) {
        ProductVariation variation = productVariationService.getVariationByCode(variationCode).orElse(null);
        if (variation != null) {
            return stockRepository.findByIdVariationStk(variation);
        }
        return List.of();
    }
}
    ...
    public byte[] generateReportStock(List<Stock> data) throws IOException {
        try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            for (Stock entity : data) {
                ...
                row.createCell(1).setCellValue(entity.getId().getStoreStk().getCode());
                row.createCell(2).setCellValue(entity.getId().getVariationStk().getProductVrt().getReference());
                row.createCell(3).setCellValue(entity.getId().getVariationStk().getColorVrt().getName());
            }
